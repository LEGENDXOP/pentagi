...
// injectActiveSteerIntoSystemPrompt injects an active operator steer directive
// at the TOP of the system prompt (right after the first line), ensuring maximum
// LLM attention. This block is replaced/removed on each system prompt refresh.
func injectActiveSteerIntoSystemPrompt(systemPrompt string, steerMsg string) string {
	steerBlock := fmt.Sprintf(
		"\n<operator_steer priority=\"CRITICAL\">\n"+
			"⚠️ ACTIVE OPERATOR OVERRIDE — IMMEDIATE COMPLIANCE REQUIRED ⚠️\n"+
			"Your operator (Master Agent) has issued the following mandatory instruction:\n\n"+
			"  %s\n\n"+
			"YOU MUST:\n"+
			"1. IMMEDIATELY change your current approach to comply with this instruction\n"+
			"2. Your NEXT tool call must be DIFFERENT from your last 3 tool calls\n"+
			"3. If you continue your previous pattern, this flow WILL BE TERMINATED\n"+
			"4. Acknowledge this override in your next reasoning\n"+
			"</operator_steer>\n",
		steerMsg,
	)

	// Remove any existing steer block first
	systemPrompt = removeStaleSteerBlock(systemPrompt)

	// Insert right after the first line (title) for maximum visibility
	firstNewline := strings.Index(systemPrompt, "\n")
	if firstNewline >= 0 {
		return systemPrompt[:firstNewline+1] + steerBlock + systemPrompt[firstNewline+1:]
	}
	return steerBlock + systemPrompt
}

// removeStaleSteerBlock removes any existing <operator_steer> block from the
// system prompt. Called when no active steer is pending to clean up old blocks.
func removeStaleSteerBlock(systemPrompt string) string {
	startTag := "<operator_steer"
	endTag := "</operator_steer>"
	startIdx := strings.Index(systemPrompt, startTag)
	endIdx := strings.Index(systemPrompt, endTag)
	if startIdx >= 0 && endIdx > startIdx {
		systemPrompt = systemPrompt[:startIdx] + systemPrompt[endIdx+len(endTag):]
	}
	return systemPrompt
}
