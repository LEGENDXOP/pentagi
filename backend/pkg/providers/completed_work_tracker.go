...
// ─── Terminal-specific thresholds (more lenient than HTTP/file-read) ─────────
// Terminal commands like `ls -la /work/` may legitimately run 2-3 times during
// normal workflow. Use Sentinel's recommended thresholds: warn at 3, block at 5.

const (
	defaultTerminalWarnThreshold  = 3 // 3rd occurrence → warning
	defaultTerminalBlockThreshold = 5 // 5th occurrence → hard block
)

func getTerminalWarnThreshold() int {
	if v := os.Getenv("WORK_TRACKER_TERMINAL_WARN_THRESHOLD"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 {
			return n
		}
	}
	return defaultTerminalWarnThreshold
}

func getTerminalBlockThreshold() int {
	if v := os.Getenv("WORK_TRACKER_TERMINAL_BLOCK_THRESHOLD"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 {
			return n
		}
	}
	return defaultTerminalBlockThreshold
}

// isTerminalFingerprint returns true if the fingerprint was generated by the
// generic terminal command normalizer (as opposed to HTTP/file-read/GraphQL).
func isTerminalFingerprint(fp string) bool {
	return strings.HasPrefix(fp, "terminal:")
}
...
func (cwt *CompletedWorkTracker) CheckReRun(toolName, toolArgs string) (bool, string) {
	...
	if fingerprint != "" {
		// Use terminal-specific (more lenient) thresholds for generic terminal
		// fingerprints. Commands like `ls -la /work/` may legitimately run 2-3
		// times, so we warn at 3 and block at 5 instead of the default 2/3.
		if isTerminalFingerprint(fingerprint) {
			warnAt = getTerminalWarnThreshold()
			blockAt = getTerminalBlockThreshold()
		}
...
func extractTerminalFingerprint(toolArgs string) string {
	...
	// Generic terminal fingerprint: catch-all for any unclassified command.
	// This closes the blind spot where commands like ls, find, id, env, whoami
	// silently fell through with no fingerprint, allowing 90+ repeats.
	if normalized := normalizeTerminalCommand(input); normalized != "" {
...
// normalizeTerminalCommand produces a stable fingerprint for any terminal command
// by extracting the base command name and its non-flag arguments.
//
// Examples:
//
//	"ls -la /root/findings/"  → "ls:/root/findings/"
//	"id"                      → "id"
//	"env | grep PATH"         → "env"  (primary command only)
//	"whoami"                  → "whoami"
func normalizeTerminalCommand(input string) string {
	primary := extractPrimaryCommand(input)
	if primary == "" {
		return ""
	}
	fields := strings.Fields(primary)
	if len(fields) == 0 {
		return ""
	}
	// Base command name (handle /usr/bin/ls → ls)
	cmd := filepath.Base(fields[0])
	// Skip leading VAR=value env-var assignments
	...
	// Collect non-flag arguments (paths, targets, etc.)
	var args []string
